...
const Navbar = ({ sidebarOpen, toggleSidebar, role = "Warden" }) => {
  ...
  return (
    <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 sticky top-0 z-[9998] h-20 flex items-center shrink-0">
          ...
          <div className="flex items-center space-x-3 pl-2">
            <div className="flex flex-col items-end text-right">
              <span className="text-slate-900 font-bold text-sm leading-tight">
                {localStorage.getItem('wardenName') || "Hostel Admin"}
              </span>
              <span className="text-indigo-500 text-[11px] font-bold uppercase tracking-wider">{role} Panel</span>
            </div>
            <div className="relative group cursor-pointer">
              <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-blue-600 flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-200 overflow-hidden">
                {localStorage.getItem('profileImage') ? (
                  <img 
                    src={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:5002'}${localStorage.getItem('profileImage')}`} 
                    alt="avatar" 
                    className="w-full h-full object-cover"
                  />
                ) : (
                  (localStorage.getItem('wardenName') || role).charAt(0).toUpperCase()
                )}
              </div>

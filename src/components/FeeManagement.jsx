...
import { hostelFeeAPI } from '../services/api';
...
const FeeManagement = () => {
    ...
    const fetchFees = async () => {
        setLoading(true);
        try {
            const response = await hostelFeeAPI.getAll();
    ...
    const handleCollectFee = async (record) => {
      ...
      `,
        ...
        if (amount) {
            try {
                const totalPaid = record.paidAmount + parseFloat(amount);
                const response = await hostelFeeAPI.update(record._id || record.id, {
                    paidAmount: totalPaid,
                    status: totalPaid >= record.totalAmount ? 'Paid' : 'Partial',

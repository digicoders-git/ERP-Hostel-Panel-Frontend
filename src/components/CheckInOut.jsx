...
import { checkInOutAPI, hostelStudentAPI } from '../services/api';
...
const CheckInOut = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const [studentRes, recordRes] = await Promise.all([
        ...
        checkInOutAPI.getAll({ date: filterDate })
      ]);
      if (studentRes.data.success) setStudents(studentRes.data.data);
      if (recordRes.data.success) setCheckInOutRecords(recordRes.data.data);
    } catch (error) {
  ...
  const handleCheckInOut = async () => {
    ...
    const student = students.find(s => (s._id || s.id).toString() === selectedStudent.toString());
    const now = new Date();
    ...
    try {
      const response = await checkInOutAPI.create(recordData);
      ...
      toast.error('Failed to record check-in/out');
    }
  };
  ...
  const getStudentStatus = (studentId) => {
    ...
    if (studentRecords.length === 0) return 'Status Unknown';
  ...
  const filteredRecords = checkInOutRecords.filter(record => {
    ...
    const rollNumber = student.rollNumber || record.rollNumber || '';
              ...
              {filteredRecords.map((record) => {
                ...
                return (
                  <tr key={record._id || record.id} className="border-b hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-gray-900">{student.name || record.studentName}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{student.rollNumber || record.rollNumber}</td>
                    <td className="px-4 py-3 text-sm text-gray-900">{student.phone || record.phone || student.contact}</td>

...
import { dashboardAPI, roomAPI, bedAllocationAPI, hostelFeeAPI, hostelStudentAPI } from '../services/api';
...
const HostelReports = () => {
  ...
  const [revenueData, setRevenueData] = useState({ total: 0, pending: 0, collectionRate: 0 });
  ...
  const fetchReportData = async () => {
    setLoading(true);
    try {
      const [dashRes, roomRes, allocRes, studentRes, feeRes] = await Promise.all([
      ...
      const occupancyRate = totalBeds > 0 ? ((allocations.length / totalBeds) * 100).toFixed(0) : 0;

      const paidFees = fees.filter(f => f.status === 'Paid').reduce((s, f) => s + (f.total || 0), 0);
      const totalFees = fees.reduce((s, f) => s + (f.total || 0), 0);
      const pendingFees = totalFees - paidFees;

      setOverviewStats([
        { title: 'Total Students', value: String(students.length), change: '', trend: 'stable', color: 'from-blue-500 to-blue-600', bgColor: 'bg-blue-50' },
        { title: 'Total Rooms', value: String(rooms.length), change: '', trend: 'stable', color: 'from-green-500 to-green-600', bgColor: 'bg-green-50' },
        { title: 'Occupancy Rate', value: `${occupancyRate}%`, change: '', trend: 'up', color: 'from-purple-500 to-purple-600', bgColor: 'bg-purple-50' },
        { title: 'Monthly Revenue', value: `₹${(paidFees / 100000).toFixed(1)}L`, change: '', trend: 'up', color: 'from-orange-500 to-orange-600', bgColor: 'bg-orange-50' },
      ]);

      setRevenueData({ total: paidFees, pending: pendingFees, collectionRate: totalFees > 0 ? ((paidFees / totalFees) * 100).toFixed(0) : 0 });

      // Room type breakdown
      const typeMap = {};
      rooms.forEach(r => {
        const type = r.roomType || r.type || 'Standard';
        if (!typeMap[type]) typeMap[type] = { type, count: 0, occupied: 0, revenue: 0 };
        typeMap[type].count++;
      });
      allocations.forEach(a => {
        const room = rooms.find(r => String(r._id) === String(a.roomId));
        if (room) {
          const type = room.roomType || room.type || 'Standard';
          if (typeMap[type]) typeMap[type].occupied++;
        }
      });
      setRoomTypeData(Object.values(typeMap));
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-purple-50 p-6 flex items-center justify-center">
        <p className="text-slate-500 text-lg">Loading report data...</p>
      </div>
    );
  }
                ...
                {roomTypeData.map((room, index) => (
                  <div key={index} className="bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-xl p-6 border border-gray-100">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between">
                        <span className="text-gray-600">Revenue:</span>
                        <span className="font-bold text-green-600">{room.occupied}/{room.count}</span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
              ...
              <div className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="bg-gradient-to-br from-green-500 to-emerald-600 rounded-2xl shadow-xl p-6 text-white">
                    ...
                    <p className="text-3xl font-bold">₹{(revenueData.total / 100000).toFixed(1)}L</p>
                  ...
                  <div className="bg-gradient-to-br from-blue-500 to-cyan-600 rounded-2xl shadow-xl p-6 text-white">
                    ...
                    <p className="text-3xl font-bold">₹{(revenueData.pending / 100000).toFixed(1)}L</p>
                  ...
                  <div className="bg-gradient-to-br from-purple-500 to-pink-600 rounded-2xl shadow-xl p-6 text-white">
                    ...
                    <p className="text-3xl font-bold">{revenueData.collectionRate}%</p>
